...
namespace OpenRA.Mods.Common.Traits
{
	...
	{
		public readonly string DefaultSequence = "idle";
		...
		public virtual IEnumerable<IActorPreview> RenderPreviewSprites(ActorPreviewInitializer init, RenderSpritesInfo rs, string image, int facings, PaletteReference p)
		{
			...
			anim.PlayRepeating(DefaultSequence);
			yield return new SpriteActorPreview(anim, WVec.Zero, 0, p, rs.Scale);
		}

		public virtual int QuantizedBodyFacings(ActorInfo ai, SequenceProvider sequenceProvider, string race)
		{
			return sequenceProvider.GetSequence(GetImage(ai, sequenceProvider, race), DefaultSequence).Facings;
		}
	...
	public class RenderSimple : RenderSprites, IAutoSelectionSize
	{
		...
		{
			DefaultAnimation.PlayRepeating(NormalizeSequence(init.Self, info.DefaultSequence));
		}
		...
		public void PlayCustomAnim(Actor self, string name)
		{
			if (DefaultAnimation.HasSequence(name))
				DefaultAnimation.PlayThen(NormalizeSequence(self, name),
					() => DefaultAnimation.PlayRepeating(NormalizeSequence(self, info.DefaultSequence)));
		}
	}
}
